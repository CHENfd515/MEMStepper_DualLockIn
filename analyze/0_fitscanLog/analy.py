...
def analyze_mems_wait_time(file_path):
    ...
    for session in sessions:
        ...
        dx_match = re.search(r'Mean Delta_Step \(dx\) = ([\d\.]+) steps', session)
        dx_val = float(dx_match.group(1)) if dx_match else 0.0

        raw_pts = re.findall(r'\(([\d\.]+),\s*([\d\.e\+-]+)\)', session)

        if not raw_pts:
            print(f"[Error] No raw (x, y) points found for {wait_time}s session.")
            continue
        ...
        # ===== 2N points div =====
        if total_pts % 2 != 0:
            print(f"[Warning] Odd number of points ({total_pts}) at {wait_time}s, skipping.")
            continue
        ...
        # loss
        fw_loss = (yf - y_theory_f) / abs(k_total)
        fb_loss = (yb - y_theory_b) / abs(k_total)
        ...
        experiment_data.append({
            ...
            'xb': xb,
            'yb': yb,
            'dy_k': (np.flip(yb) - yf) / abs(k_total),
    ...
    if num_sessions ==1:
        ...
        fig = plt.figure(figsize=(4*num_sessions, 12)) 
    plt.rc('axes', axisbelow=True)
    fig.patch.set_facecolor('white')
    rows, cols = 5, num_sessions 
    colors = plt.cm.viridis(np.linspace(0, 0.8, num_sessions))
    
    all_y = np.concatenate([d['yf'] for d in experiment_data] + [d['yb'] for d in experiment_data])
    ...
    for i, data in enumerate(experiment_data):
        ...
        ax.scatter(data['xf'], data['yf'], color=colors[i], marker='o', s=30, label='Forward')
        ax.scatter(data['xb'], data['yb'], color='red', marker='x', s=40, label='Backward')
        ax.set_ylim(y_min_val, y_max_val)
        ax.set_title(f'Wait: {data["wait_time"]}s', fontweight='bold', fontsize=10)
        ax.set_xlabel('Position (Step)')
        ax.legend(fontsize=8)
        if i == 0: ax.set_ylabel('uV', fontweight='bold')
    ...
    x_pos, x_indices = experiment_data[0]['xf'], np.arange(len(experiment_data[0]['xf']))
    ...
    for i, data in enumerate(experiment_data):
        ...
        bars = ax_err.bar(x_indices + offset, plot_vals, width=bar_width, color=colors[i], alpha=0.8)
    ...
    ax_err.set_ylim(bottom=log_floor, top=max_pct * 30) # Increase upper limit to fit horizontal text
    ax_err.set_title('abs (Backward - Forward) (Step %)', fontsize=12, fontweight='bold')
    ...
    ax_err.grid(True, which='major', linestyle='-', alpha=0.4) # Only Major Grid
    ...
    bars_trend = ax_trend.bar(wt_labels, dx_pct_vals, color='#3498db', alpha=0.7, width=0.4)
    ...
    ax_trend.set_title('mean (abs (Backward - Forward)) (Step %)', fontsize=12, fontweight='bold')
    ax_trend.grid(True, which='major', linestyle='-', alpha=0.4) # Only Major Grid
    ...
    bars_b = ax_lin.bar(x_wt + 0.2, [d['lin_b'] for d in experiment_data], width=0.4, label='Backward', color='darkred', alpha=0.7)
    ...
    ax_lin.set_title('Non-linearity Error (%)', fontsize=12, fontweight='bold')
    ax_lin.set_xticks(x_wt)
    ax_lin.set_xticklabels(wt_labels)
    ax_lin.set_ylabel('Error (%)', fontweight='bold')
    ax_lin.set_xlabel('Wait Time')
    ax_lin.grid(True, which='major', linestyle='-', alpha=0.3) # Only Major Grid
    ...
    plt.savefig('scan_analy.svg', format='svg', bbox_inches='tight')
    plt.savefig('scan_analy.pdf', format='pdf', bbox_inches='tight')
    # plt.show()
